...
GPIO_TypeDef* COM_TX_PORT[COMn] = {GPIOA,GPIOD,GPIOB}; 
GPIO_TypeDef* COM_RX_PORT[COMn] = {GPIOA,GPIOD,GPIOB}; 
const uint32_t COM_USART_CLK[COMn] = {RCC_APB2Periph_USART1, RCC_APB1Periph_USART2, RCC_APB1Periph_USART3};
const uint32_t COM_TX_PORT_CLK[COMn] = {RCC_AHB1Periph_GPIOA, RCC_AHB1Periph_GPIOD, RCC_AHB1Periph_GPIOB};
const uint32_t COM_RX_PORT_CLK[COMn] = {RCC_AHB1Periph_GPIOA, RCC_AHB1Periph_GPIOD, RCC_AHB1Periph_GPIOB};
const uint16_t COM_TX_PIN[COMn] = {GPIO_Pin_9, GPIO_Pin_5,GPIO_Pin_10};
const uint16_t COM_RX_PIN[COMn] = {GPIO_Pin_10, GPIO_Pin_6,GPIO_Pin_11};
const uint16_t COM_TX_PIN_SOURCE[COMn] = {GPIO_PinSource9,GPIO_PinSource5,GPIO_PinSource10};
const uint16_t COM_RX_PIN_SOURCE[COMn] = {GPIO_PinSource10,GPIO_PinSource6,GPIO_PinSource11};
...
void DBG_InitUart(COM_TypeDef COM,u32 UsartBaudRate)
{
  ...
  /* CPU的小缺陷：串口配置好，如果知己Send，则第1个字节发送不出去。如下语句解决第1个字节无法正确发送出去的问题*/
...
void DBG_USART_SendStr(USART_TypeDef* USARTx,const char * data)
{
		...
		for (i=0; i<len; i++)
		{
			DBG_USART_SendByte(USARTx,data[i]);
		}
}
/**
...
/**
  ...
  *	@使用范例:  USART_SedndByte(USART2,0xAA);                
  */
void DBG_USART_SendHex(USART_TypeDef* USARTx,uint8_t byte)
{ 
//数字+30  大写字母+37  
		u8 temp_high;
		u8 temp_low;
	  ...
	  //保留低位
		...
		DBG_USART_SendByte(USARTx,temp_high);
		DBG_USART_SendByte(USARTx,temp_low);
}
void DBG_printEnter(void)
{
	DBG_USART_SendByte(USART1,0x0D); //
	DBG_USART_SendByte(USART1,0x0A); //回车
}
...
{ 
   	...
   	return (uint16_t)(USARTx->DR & (uint16_t)0x01FF);					   //接收一个字符	   
}
